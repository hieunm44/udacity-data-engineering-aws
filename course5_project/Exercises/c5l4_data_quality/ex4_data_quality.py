...
# Thêm một task check_greater_than_zero để kiểm tra số lượng records $> 0$.
# Trong task load_trip_data_to_redshift, ta đặt một SLA (Service Level Agreement). Đó là một cơ chế trong Airflow cho phép set specific time constraints cho task cụ thể.
# Nó giúp theo dõi và thực thi tính kịp thời của việc thực hiện task.